...
function indianMobile(value, label) {
  ...
  if (!/^[6-9]\d{9}$/.test(mobile)) {
    throw validationError(`${label} must be a valid 10-digit Indian mobile number`);
  }
  return mobile;
}
...
async function submit(input = {}) {
  ...
  const duplicateQuery = {
    ...
    $or: [{ normalizedMobile: mobile }],
  };
  if (normalizedEmail) duplicateQuery.$or.push({ email: normalizedEmail });
  const duplicate = await ProviderJoinRequest.findOne(duplicateQuery)
    .select({ providerJoinRequestId: 1, status: 1 })
    .lean();
  if (duplicate) {
    return {
      accepted: true,
      duplicate: true,
      providerJoinRequestId: duplicate.providerJoinRequestId,
    };
  }

  let row;
  try {
    row = await ProviderJoinRequest.create({
    ...
    if (error?.code === 11000) {
      const existing = await ProviderJoinRequest.findOne({
        normalizedMobile: mobile,
        status: { $in: OPEN_STATUSES },
      }).select({ providerJoinRequestId: 1 }).lean();
      return {
        accepted: true,
        duplicate: true,
        providerJoinRequestId: existing?.providerJoinRequestId || "",
      };
    }
    throw error;
  }
