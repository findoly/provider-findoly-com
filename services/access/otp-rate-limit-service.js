...
function rateLimitModel() {
  return require("../../models/ProviderOtpRateLimit");
}
...
function settings() {
  return {
    resendSeconds: integerSetting(process.env.PROVIDER_OTP_RESEND_SECONDS, 30, 1, 3600),
    maxSendsPerWindow: integerSetting(process.env.PROVIDER_OTP_MAX_SENDS_PER_WINDOW, 2, 1, 20),
    windowSeconds: integerSetting(process.env.PROVIDER_OTP_RATE_WINDOW_SECONDS, 60, 10, 3600),
  };
}
...
function rateLimitError(waitSeconds) {
  ...
  error.status = 429;
  error.code = "PROVIDER_OTP_SEND_RATE_LIMIT";
...
async function claimSendSlot(mobile, now = new Date()) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    const ProviderOtpRateLimit = rateLimitModel();
    const state = await ProviderOtpRateLimit.findOne({ mobile }).lean();
    ...
    if (!state) {
      try {
        await ProviderOtpRateLimit.create({
          mobile,
    ...
    const result = await ProviderOtpRateLimit.updateOne(
  ...
  const ProviderOtpRateLimit = rateLimitModel();
  const latest = await ProviderOtpRateLimit.findOne({ mobile }).lean();
...
async function releaseSendSlot(mobile, requestId, now = new Date()) {
  ...
  const ProviderOtpRateLimit = rateLimitModel();
  const state = await ProviderOtpRateLimit.findOne({ mobile, lastRequestId: requestId }).lean();
  if (!state) return;

  const sendCount = Math.max(0, Number(state.sendCount || 0) - 1);
  if (sendCount === 0) {
    await ProviderOtpRateLimit.deleteOne({ _id: state._id, lastRequestId: requestId });
    return;
  }

  await ProviderOtpRateLimit.updateOne(
